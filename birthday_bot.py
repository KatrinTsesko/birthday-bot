...
class BirthdayBot:
    ...
    async def add_birthday(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            self.save_birthdays()
            await update.message.reply_text(
                f"✅ {name} добавлен(а): {day:02d}.{month:02d}"
            )
        except ValueError:
        ...
        try:
            ...
            data = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "Ты пишешь теплые поздравления для коллег.",
                    },
    ...
    async def send_birthday_greetings(self, context: ContextTypes.DEFAULT_TYPE):
        ...
        birthdays_today = {
            name: date for name, date in self.birthdays.items() if date == today
        }
        messages = []
...
if __name__ == "__main__":
    ...
    # ⚡️ Запуск webhook — без asyncio.run()
    app.run_webhook(
        listen="0.0.0.0",
